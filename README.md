<h1 align="center"> 👋🏻 Hello World! I'm Krasimir Naydenov</h1>
<h3 align="center">C# Software Engineering Student • Learning the .NET technologies</h3>

 <p align="left">
      <a href="https://github.com/krasimirnyv?tab=followers">
         <img alt="followers" title="Follow me on Github" src="https://custom-icon-badges.demolab.com/github/followers/krasimirnyv?color=236ad3&labelColor=1155ba&style=for-the-badge&logo=person-add&label=Follow&logoColor=white"/></a>
   </p>

## About me

Highly motivated student, deeply immersed in the world of **C# .NET** technologies, seeking new knowledge and skills to carefully apply in my future career as a **Software Engineer**.

I stand out with my artistry as an audio-visual creator. Working in film productions has developed my ability to collaborate effectively in large teams and adapt to different situations and challenges.

**The screen arts are my passion. Programming is my dream.**

- 🎯 Current focus: **C#**, **ASP.NET Core**, **EF Core**, **SQL Server**
- 🧭 Engineering principles that I strive to follow: **SOLID**, **Clean Architecture**, **Clean Code**, **Separation of Concerns**, **Dependency Injection / IoC**
- 🚀 Career: currently in an intensive learning phase; but **soon** I’ll start actively applying for **Internship / Junior** roles
- 🎥 Fun fact: I’m a **retired cinematographer**
- ☕️ PS: I’m also self-studying **Java**

---

## Featured projects - Portfolio

### 🎬 WRAP - Film Production Management System
A web platform for managing film productions - organizing scripts, production planning and scheduling, teams, and assets.

- 🔭 Currently working on: **WRAP**
- Key features:
  - **Script editor** (editing/structuring scripts as a core workflow)
  - **Two user roles: Crew and Cast** (role-driven experience and permissions)
  - **Scheduling** (planning and coordinating production work)

The project is currently in active development. While some features are still in the "visionary" stage, the core architecture is designed to handle the dynamic nature of movie sets.

Can't wait to shout: “It’s a WRAP!”

Repo: <a href="https://github.com/krasimirnyv/Wrap-FilmProductionManagementSystem-ASP.NET-Project">Check the WRAP project</a>

---

### 🐍 Snake Game with Custom DI Framework
Classic Snake in C#, built on top of a custom-made **Dependency Injection / IoC** framework.

Highlights:
- A practical showcase of **DI / IoC** and **separation of concerns**
- Clean OOP structure (interfaces, responsibilities, extensibility)

Repo: <a href="https://github.com/krasimirnyv/Snake-Game-with-custom-DI-Framework">Check the 🐍 game</a>

---

## Courseworks and homeworks from Software University - my learning progress

These repositories represent my learning path and growth - what I’ve studied, how I write code, and how I approach problems.

- C# Web: <a href="https://github.com/krasimirnyv/SoftUni-CSharp-Web">Link to my ASP.NET Core repo</a>
- C# DB: <a href="https://github.com/krasimirnyv/SoftUni-CSharp-DB">Link to my Entity Framework Core repo</a>
- C# OOP: <a href="https://github.com/krasimirnyv/SoftUni-CSharp-OOPModule">Link to my Object-Oriented Programming repo</a>
- C# Advanced: <a href="https://github.com/krasimirnyv/SoftUni-CSharp-AdvancedModule">Link to my Advanced Module repo</a>
- C# Fundamentals: <a href="https://github.com/krasimirnyv/SoftUni-CSharp-FundamentalsModule">Link to my Fundamentals Module repo (regular)</a>
  | <a href="https://github.com/krasimirnyv/MoreExercises-CSharp-Fundamentals">Link to my Fundamentals Module repo (additional)</a> 

---

## My tech stack

**Top technologies:**
- **Core:** C#, .NET, ASP.NET Core, Entity Framework Core, SQL Server  
- **Also familiar / exploring:** JavaScript, HTML, CSS, Bootstrap, Docker, Kubernetes, CI/CD, Java

<div align="center">
	<code><img width="60" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/c%23.png" alt="C#" title="C#"/></code>
	<code><img width="60" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/_net_core.png" alt=".NET Core" title=".NET Core"/></code>
	<code><img width="60" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/mssql.png" alt="MSSQL" title="MSSQL"/></code>
  </div>

<div align="center">
	<code><img width="40" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/javascript.png" alt="JavaScript" title="JavaScript"/></code>
	<code><img width="50" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/html.png" alt="HTML" title="HTML"/></code>
	<code><img width="50" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/css.png" alt="CSS" title="CSS"/></code>
	<code><img width="50" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/bootstrap.png" alt="Bootstrap" title="Bootstrap"/></code>
	<code><img width="45" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/git.png" alt="Git" title="Git"/></code>
	<code><img width="50" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/github.png" alt="GitHub" title="GitHub"/></code>
	<code><img width="50" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/docker.png" alt="Docker" title="Docker"/></code>
	<code><img width="45" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/kubernetes.png" alt="Kubernetes" title="Kubernetes"/></code>
	<code><img width="50" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/ci_cd.png" alt="CI/CD" title="CI/CD"/></code>
	<code><img width="50" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/java.png" alt="Java" title="Java"/></code>
</div>

---

## Connect with me

- LinkedIn: **<a href="https://www.linkedin.com/in/krasimir-naydenov-b6aa2b241">Visit my profile</a>**
- Email: **krasi.photography@gmail.com**

---

## GitHub stats

<p>
  <img align="center" src="https://github-readme-streak-stats.herokuapp.com/?user=krasimirnyv" alt="Krasi's GitHub stats" />
</p>
